...
`transition()` works well with [`transformCss`](/api/transformCss/). Normally, working with CSS transitions and transforms is a spaghetti mess of vendor-prefixing. Pablo abstracts this away so that vendor-prefixed properties like `-webkit-transform` can be passed in unprefixed:
